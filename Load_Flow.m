...
clc
format short;
m=load('loaddata25bus.m');
l=load('linedata25bus.m');
...
figure
plot(Pl*100)
xlabel('Bus'),ylabel('MW'),title('Power Loss')
legend('Base')
...
disp('            Results                    ')
disp('---------------------------------------')
disp(['Real Power-Loss (MW):        ' num2str(sum(Pl)*100)]);
disp(['Reactive Power-Loss (MVar):  ' num2str(sum(Ql)*100)]);
disp('---------------------------------------');